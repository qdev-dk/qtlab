...
class RhodeSchwartz_ZVCE_1127_8600_50(Instrument):
    ...
    def __init__(self, name, address, reset=False):
        ...
        #self.add_parameter('span_frequency', type=types.FloatType,
        #                   flags=Instrument.FLAG_GETSET|Instrument.FLAG_GET_AFTER_SET,
        #                   units=self._freq_unit_symbol, minval=0.020, maxval=8000)
...
# --------------------------------------
    ...
    def get_data(self):
        ...
        raw = self._visainstrument.ask('TRAC? CH1DATA')
...
#       return eval('[' + self._visainstrument.ask('TRAC:STIM? CH1DATA') + ']')    
    ...
    def do_get_resolution_bandwidth(self): #in Hz
        ...
        return float(self._visainstrument.ask('BAND?'))/self._freq_unit
    ...
    def do_set_numpoints(self,numpoints):
        logging.debug('Setting sweep points to %s' % numpoints)
        self._visainstrument.write('SWE:POIN %s' % numpoints)
        self.get_sweeptime()
    ...
    def do_set_resolution_bandwidth(self,resolution_bandwidth): #in Hz
        '''
        Don't set too low (see FSL). Can be manually set up to 10Hz.
        ...
        self._visainstrument.write('BAND %sHz' % resolution_bandwidth)
        self.get_sweeptime()
